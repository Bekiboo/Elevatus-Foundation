<script>
  import { blogEditMode } from '$lib/stores'
  import ArrowSvg from '$lib/svg/symbols/ArrowSvg.svelte'
...
<button
  class="w-fit ml-4 my-2 bg-white rounded-full p-1 hover:bg-honey"
  on:click={() => $blogEditMode = 'dashboard'}>
  <ArrowSvg />
</button>
...
<section class="px-4 max-w-4xl">
  <form on:submit|preventDefault={formatData}>
    ...
    <button
      class="bg-indigo-500 mt-4 py-1 px-2 rounded-md  hover:bg-indigo-400 duration-100"
